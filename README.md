...
- Generate local coverage with `go test -coverpkg="$(go list ./... | paste -sd, -)" ./... -v -count=1 -coverprofile=coverage/coverage.out -timeout=5m` and create an HTML report with `go tool cover -html=coverage/coverage.out -o coverage/coverage.html`.
