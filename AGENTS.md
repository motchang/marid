...
- Static analysis and linter checks must run before tests; use the checks introduced in this repository (`golangci-lint run ./...` and the cccc/Octocov workflow path where applicable).
- Tests are required for new behavior; add table-driven tests when practical. After static analysis and linter checks pass, run `go test ./...` before submitting.
